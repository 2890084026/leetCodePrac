...
//   return maxArea
// }
var largestRectangleArea = function largestRectangleArea(heights) {
  heights.push(0);
  var stack = [],
      len = heights.length,
      maxArea = 0;
  ...
  for (i = 0; i < len; i++) {
    while (stack.length && heights[i] < heights[stack.at(-1)]) {
      var cur = stack.pop();
      var h = heights[cur];
      var left = stack.length ? stack.at(-1) : -1;
      var w = i - 1 - left;
      maxArea = Math.max(maxArea, h * w);
    }
...
var largestRectangleArea = function largestRectangleArea(heights) {
  ...
  for (var _i = 0; _i < len; _i++) {
    while (stack.length && heights[_i] < stack.at(-1)) {
      var cur = stack.pop();
      var h = heights[cur];
      var left = stack.length ? stack.at(-1) : -1;
      var w = _i - 1 - left;
